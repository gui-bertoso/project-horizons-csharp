...
public partial class SaveCreationLoading : Control
{
	...
	private void CacheThreadSafeData()
	{
		...
		if (EnemiesManager.I != null && EnemiesManager.I.enemyPathsToBiome != null)
		{
			foreach (var pair in EnemiesManager.I.enemyPathsToBiome)
			{
	...
	private Godot.Collections.Dictionary SerializeChunk(DeltaBakeChunkData chunk)
	{
		return new Godot.Collections.Dictionary
		{
			...
			{ "canopy", PackTiles(chunk.CanopyTiles) },
		};
	}
	...
	private DeltaThreadGenerationResult BuildChunksFromWorldData(DeltaThreadWorldData world)
	{
		...
		foreach (var pair in world.Canopy) AddTileToChunk(result.Chunks, pair.Key, pair.Value, DeltaBakeLayerType.Canopy);
	...
	private void AddTileToChunk(Dictionary<Vector2I, DeltaBakeChunkData> chunks, Vector2I cell, Vector2I atlas, DeltaBakeLayerType layerType)
	{
		...
		switch (layerType)
		{
			...
			case DeltaBakeLayerType.Canopy:
				chunk.CanopyTiles.Add(new DeltaBakeTileData(cell, atlas));
				break;
		}
	}
	...
	private enum DeltaBakeLayerType { Ground, SmallDetails, MediumDetails, Objects, Shadows, Canopy }

	private sealed class ThreadGenerationContext
	{
		private readonly SaveCreationLoading _owner;
		...
		public ThreadGenerationContext(SaveCreationLoading owner, int levelId, int biomeId)
		{
			_owner = owner;
			_levelId = levelId;
			_biomeId = biomeId;
		...
		public void GenerateTreesPass(DeltaThreadWorldData world)
		{
			...
			for (int y = -halfY; y < halfY; y += safeTreeStep)
			{
				for (int x = -halfX; x < halfX; x += safeTreeStep)
				{
					...
					{
						world.Objects[cell] = _owner.GetTreeTrunkAtlas(atlas);
						world.Shadows[cell + _owner.ShadowOffset] = _owner.TreeShadowAtlas;
						Vector2I canopyAtlas = _owner.GetTreeCanopyAtlas(atlas);
						if (canopyAtlas != Vector2I.Zero)
							world.Canopy[cell] = canopyAtlas;
						_treeSet.Add(cell);
					}
				}
			}
